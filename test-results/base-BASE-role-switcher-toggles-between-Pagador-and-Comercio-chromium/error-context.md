...
- Name: base.spec.js >> BASE: role switcher toggles between Pagador and Comercio
- Location: tests/e2e/base.spec.js:135:1
...
Locator: locator('text=Billetera Principal')
Expected: visible
Timeout: 10000ms
Error: element(s) not found

Call log:
  - Expect "toBeVisible" locator('text=Billetera Principal') with timeout 10000ms
  - waiting for locator('text=Billetera Principal')
...
> 140 |   await expect(page.locator('text=Billetera Principal')).toBeVisible();
  ...
  189 | });
  190 | 
  191 | // ─── BASE TEST: settings user profile shows ─────────────────────────────
  192 | test('BASE: settings sheet shows user info', async ({ page }) => {
  193 |   await clearAppState(page);
  194 |   await page.getByRole('button', { name: /Pagador/i }).click();
  195 | 
  196 |   const settingsBtn = page.locator('[title="Configuración y Perfil"]');
  197 |   await settingsBtn.click();
  198 | 
  199 |   // Should show the demo user email
  200 |   await expect(page.locator('text=pollar.io')).toBeVisible({ timeout: 3000 });
  201 | });
  202 | 
```